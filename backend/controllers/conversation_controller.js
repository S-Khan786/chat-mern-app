...
const validIds = (ids) =>
  Array.isArray(ids) &&
  ids.length > 0 &&
  ids.every((id) => mongoose.isValidObjectId(id));
...
export const createGroupConversation = async (req, res) => {
  try {
    ...
    const allParticipants = [
      ...new Set([req.user._id.toString(), ...participants.map(String)]),
    ];
    if (!name || name.length > 120)
      return res
        ...
        .json({
          success: false,
          message: "A group name between 1 and 120 characters is required",
        });
    if (!validIds(allParticipants) || allParticipants.length < 2)
      return res
        .status(400)
        .json({
          success: false,
          message: "A group needs at least two valid participants",
        });
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to create group" });
  }
};
...
export const getConversation = async (req, res) => {
  try {
    ...
    if (!isParticipant(conversation, req.user._id))
      return res
        .status(403)
        .json({ success: false, message: "You are not a conversation member" });
...
export const updateGroupParticipants = async (req, res) => {
  try {
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid conversation or user" });
    }
    const conversation = await Conversation.findById(req.params.id);
    ...
    if (!conversation.admins.some((admin) => admin.equals(req.user._id)))
      return res
        .status(403)
        .json({
          success: false,
    ...
    if (conversation.participants.length < 2)
      return res
        ...
        .json({
          success: false,
          message: "A group needs at least two participants",
        });
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to update group members" });
  }
};
