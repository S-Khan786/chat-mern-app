...
const MessageContainer = ({ onBackUser }) => {
  ...
  return <section className="flex h-full min-w-0 flex-1 flex-col">
    <header className="theme-surface relative flex shrink-0 items-center gap-2 border-b border-white/10 px-3 py-3 sm:gap-3 sm:px-6 sm:py-3.5">
      ...
      {isGroup && showMembers && <div className="theme-surface absolute right-3 top-14 z-10 w-[min(18rem,calc(100vw-2rem))] rounded-2xl border p-3 shadow-2xl sm:right-4 sm:top-16"><p className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Group members</p><div className="max-h-56 space-y-2 overflow-y-auto">{(selectedConversation.participants || []).map((participant) => { const memberName = participant.username || participant.fullname || String(participant); const memberId = participant._id || participant; return <div key={memberId} className="flex items-center gap-2"><div className="relative grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-violet-400/30 to-cyan-400/20 text-xs font-bold text-violet-200"><span>{memberName.charAt(0).toUpperCase()}</span>{participant.profilePic && <img src={participant.profilePic} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} className="absolute h-8 w-8 rounded-lg object-cover" />}</div><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{String(memberId) === String(authUser?._id) ? "You" : memberName}</p>{selectedConversation.admins?.some((admin) => String(admin?._id || admin) === String(memberId)) && <p className="text-[10px] text-violet-300">Admin</p>}</div></div>; })}</div></div>}
    </header>
