  ...
  return <section className="flex h-full min-w-0 flex-1 flex-col">
    ...
    <div ref={messageListRef} className="theme-canvas scrollbar min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(circle_at_center,rgba(255,128,108,.07),transparent_36rem)] px-3 py-4 sm:px-6 sm:py-5">
      ...
      {!loading && visibleMessages.map((message) => {
        ...
        return (
          <div key={message._id} className={`msg-row group mb-4 flex ${mine ? "justify-end" : "justify-start"}`}>
            <div className={`flex max-w-[82%] flex-col sm:max-w-[68%] ${mine ? "items-end" : "items-start"}`}>
              ...
              <div className="msg-actions flex items-center gap-2.5 px-1 pt-1">
                ...
                {mine && <span className="text-[10px] capitalize text-slate-500">{receipt?.status || "sent"}</span>}
                <span className="mx-0.5 h-3 w-px bg-white/10" />
                <button type="button" onClick={() => setReplyMessage(message)} aria-label="Reply to message" className="text-xs text-slate-400 transition hover:text-white"><FaReply /></button>
                {mine && !message.deletedAt && (
                  <>
                    <button type="button" onClick={() => startEdit(message)} aria-label="Edit message" className="text-xs text-slate-400 transition hover:text-white"><FiEdit2 /></button>
                    <button type="button" onClick={() => deleteMessage(message)} aria-label="Delete message" className="text-xs text-slate-400 transition hover:text-rose-300"><FiTrash2 /></button>
                  </>
                )}
                <span className="flex gap-1">
                  {["👍", "❤️", "😂"].map((emoji) => (
                    <button key={emoji} type="button" onClick={() => axios.put(`/api/message/${message._id}/reaction`, { emoji })} className="text-xs opacity-70 transition hover:scale-125 hover:opacity-100">{emoji}</button>
                  ))}
                </span>
              </div>
              {message.reactions?.length > 0 && <div className="mt-1 rounded-full bg-white/10 px-2 py-0.5 text-xs">{message.reactions.map((reaction) => reaction.emoji).join(" ")}</div>}
            </div>
          </div>
        );
      })}
